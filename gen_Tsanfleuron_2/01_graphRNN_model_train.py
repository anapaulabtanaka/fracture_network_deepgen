...
use_bfs = True
pos_attr = 'pos'  # use spatial BFS ordering to reduce adjacency-matrix bandwidth
max_n_nodes = None
max_prev_node = None
calc_max_prev_node_kwargs={'nsample':10000, 'quantile':1.0, 'seed':134}

data_set = Graph_sequence_sampler_data_set(
    G_list, G_nsample, use_bfs=use_bfs, pos_attr=pos_attr,
...
# - 2. Adam
lr_G = 0.001          # learning rate; default: lr=0.001
...
lr_scheduler_G = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer_G, T_0=T_0_G, T_mult=T_mult_G, eta_min=eta_min_G, last_epoch=-1)
# Essentially, with eta_max = lr_init, at epoch t, the learning rate is set to
...
# - 2. Adam
lr_E = 0.001           # learning rate; default: lr=0.001
...
lr_scheduler_E = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer_E, T_0=T_0_E, T_mult=T_mult_E, eta_min=eta_min_E, last_epoch=-1)
# Essentially, with eta_max = lr_init, at epoch t, the learning rate is set to
#    eta_t = eta_min + 1/2 * (eta_max - eta_min) * (1 + cos(T_cur/T_i*pi))
# where
#    T_cur: the number of epochs since the last restart
...
# Train
t1 = time.time()
patience = 20    # epochs with no improvement before stopping (set to None to disable)
min_delta = 1e-5 # minimum loss decrease to count as improvement
...
n_epochs_trained = len(loss)
nskip_beg = int(0.3*n_epochs_trained)
nskip_end = 0
...
plt.figure(figsize=figsize)
plt.plot(np.arange(nskip_beg, n_epochs_trained-nskip_end), loss[nskip_beg:n_epochs_trained-nskip_end], ls='-', color=color_train, label='loss')
plt.yscale('log')
plt.xlabel('epoch')
plt.legend()
plt.grid()
